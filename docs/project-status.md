...
- **数据库**: MySQL + SQLAlchemy async（**asyncmy**；`.env` 中写 `mysql+aiomysql://` 时启动会规范为 `mysql+asyncmy://`）
...
│       │   ├── logging.py               # 日志配置
│       │   ├── database.py              # SQLAlchemy async 引擎（asyncmy）、连接池参数、URL 规范
...
│       │   └── persist_service.py       # upsert_video / record_download_history / record_ai_task / **persist_ai_task_for_url**（写库重试）
...
│       ├── api/
│       │   ├── video.ts                 # 视频 API 客户端
│       │   ├── ai.ts                    # AI API（SSE 问答、额度、**extractApiErrorMessage** 解析 500/503 纯文本或 JSON detail）
...
├── extension/                           # Chrome MV3 插件（详见 docs/chrome插件开发.md）
│   ├── manifest.json
│   ├── build.mjs                        # Vite 构建 popup → dist/popup；esbuild content/background
│   └── src/
│       ├── popup/                       # Vue 3 弹窗（解析、下载、登录、AI）
│       ├── content/                     # 按平台注入（YouTube / B 站 / 抖音 / TikTok）
...
└── docs/
    ...
    ├── chrome插件开发.md                 # 插件目标与目录说明（与仓库实现对齐节见文内）
...
| `translate:v2:{url_hash}:{lang}:{content_sha}` | 6 小时 | 字幕翻译结果；**content_sha** 为当前 `texts[]` 序列化后的摘要，避免仅按 URL 缓存导致与最新字幕行错位 |
...
```env
DATABASE_URL=mysql+asyncmy://omni:omni123@localhost:3306/omni_video
...
TASK_MODE=local                   # 默认 local（开发，无 RQ）；生产设 queue 并起 RQ worker
...
OPENAI_MODEL=deepseek-chat
TRANSLATE_MAX_OUTPUT_TOKENS=8192     # 可选；字幕翻译单次输出上限，过小易截断 JSON 导致缺行
...
**macOS**：若下载时出现「Python 意外退出」、`fork pre-exec` 弹窗，不要用 `python -m uvicorn main:app`（uvicorn 会先加载，环境变量设太晚）。请：

```bash
cd backend && source venv/bin/activate && export PYTHONPATH=.
python dev_uvicorn.py                    # macOS 默认 **无** --reload，端口 8000
# 或：./scripts/dev-uvicorn-macos.sh
# queue 模式 Worker：python rq_worker.py worker video_tasks
```

其它系统可继续用 uvicorn 命令行。

```bash
# 1. 后端（非 macOS 或已按上文处理时）
cd backend
...
| **Chrome 浏览器插件** | Manifest V3：`extension/`（`npm run build` 产出 `dist/`）popup（Vue）+ content script；`shared/videoUrlPatterns.ts` 与后端一致识别视频页（含抖音 **`/jingxuan?modal_id=`** 等）；登录 JWT 存 `chrome.storage`；链接「在网站上打开」带 `?url=` | ✅ MVP 已可用 |
...
- 异步驱动连接绑定到事件循环，跨循环复用会报错；生产使用 **asyncmy** + 单进程 uvicorn 事件循环
- 解决：集成测试尽量在同一 loop 内跑；脚本验证表结构可用同步 **pymysql** 直连，避免与应用共享 async engine

### AI 路由与数据库稳定性（2026-03）

- **鉴权**：`get_optional_user` 仅吞 JWT 解析类错误；请求头带非空 Bearer 但无法解析用户时，`check_ai_llm_access` 返回 **401**，避免误走匿名 IP 限额。
- **会话时长**：`summary` / `mindmap` / `chat` / `translate` 在 `get_video_context`、LLM 等慢逻辑前后使用**短 `async_session()`**，避免长占连接池（远程 MySQL 易 `Lost connection` / packet 异常）。
- **引擎**：`database.py` 使用 `pool_pre_ping`、`pool_recycle`、`pool_reset_on_return`，`pool_size`/`max_overflow` 可调。
- **写库**：`persist_ai_task_for_url` 对 **`DBAPIError`** 重试多次；`main.py` 注册 **`DBAPIError` → 503 JSON**，避免前端只收到纯文本 `Internal Server Error`。
- **字幕翻译**：`ai_service` 小批次并行、输出长度与质量重试；缓存键含字幕正文指纹（见第九节）。

### 前端首页 `?url=` 深链

- 插件或外链打开 `http://localhost:5173/?url=encodeURIComponent(视频链接)` 时，`Home.vue` 会**回填解析输入框**并 `replace` 掉 query，便于从插件跳到网站继续操作。
